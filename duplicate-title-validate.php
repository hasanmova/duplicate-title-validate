...
function duplicate_title_checks_callback() {
		...
		function title_checks() {
			...
			if($results) {
				return "<span style='color:red'>". _e( 'Please enter another title it\'s already exists' , 'dublicate-title-validate' ) ." </span>";
             //لطفا عنوان دیگری را انتخاب کنید این عنوان ثبت شده است .   
			} else {
				return '<span style="color:green">'._e('This title is unique' , 'dublicate-title-validate').'</span>';
			}
            //عنوان انتخاب شده مورد تایید است .
		}		
		echo title_checks();
		die();
	}
...
function not_published_error_notice() {
    if(isset($_GET['wallfaerror']) == 1 ){
                   ?>
               <div class="updated">
               <p style='color:red' ><?php _e('Posted considered to have been released as a duplicate' , 'dublicate-title-validate') ?></p>
               </div>
               <?php
               //نوشته مورد نظر به دلیل داشتن عنوان تکراری منتشر نشد 
        }}
